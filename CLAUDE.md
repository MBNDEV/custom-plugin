...
This is a WordPress plugin. All PHP must pass `composer run lint` only do this after all edits have been made (PHPCS with WordPress-Core, WordPress-Extra, WordPress-Docs).
...
- All user-facing strings use `__()` or `_e()` with text domain `custom-plugin`
- Enqueue scripts via `wp_enqueue_scripts` — no inline `<script>` tags

## Lint

```bash
composer run lint:run    # fix twice, then check
```