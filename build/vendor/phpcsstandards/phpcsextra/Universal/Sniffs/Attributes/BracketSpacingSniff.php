...
final class BracketSpacingSniff implements Sniff
{
    ...
    public function processOpener(File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->ignoreNewlines === \true && $tokens[$stackPtr]['line'] !== $tokens[$nextNonWhitespace]['line']) {
        ...
        SpacesFixer::checkAndFix($phpcsFile, $stackPtr, $nextNonWhitespace, $this->spacing, 'Expected %s after the attribute block opener. Found: %s.', 'SpaceAfterOpener', 'error', 0, self::METRIC_NAME);
    }
    /**
    ...
    {
        $tokens = $phpcsFile->getTokens();
        $previousNonWhitespace = $phpcsFile->findPrevious(\T_WHITESPACE, $stackPtr - 1, null, \true);
        if ($this->ignoreNewlines === \true && $tokens[$stackPtr]['line'] !== $tokens[$previousNonWhitespace]['line']) {
        ...
        SpacesFixer::checkAndFix($phpcsFile, $previousNonWhitespace, $stackPtr, $this->spacing, 'Expected %s before the attribute block closer. Found: %s.', 'SpaceBeforeCloser', 'error', 0, self::METRIC_NAME);
    }
}
