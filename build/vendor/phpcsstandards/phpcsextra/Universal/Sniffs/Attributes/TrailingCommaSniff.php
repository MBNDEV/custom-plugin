 ...
 */
namespace _PhpScoperdd275a4cd119\PHPCSExtra\Universal\Sniffs\Attributes;
...
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\AttributeBlock;
/**
...
final class TrailingCommaSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $beforeCloser = $phpcsFile->findPrevious(Tokens::$emptyTokens, $closer - 1, null, \true);
        ...
        if ($tokens[$opener]['line'] === $tokens[$closer]['line']) {
            ...
            if ($tokens[$beforeCloser]['code'] === \T_COMMA) {
                ...
                $fix = $phpcsFile->addFixableError('Trailing comma is not allowed in a single-line attribute block', $beforeCloser, 'ForbiddenSingleLine');
                if ($fix === \true) {
        ...
        if ($attributeCount > 1) {
            ...
            if ($tokens[$beforeCloser]['code'] !== \T_COMMA) {
                ...
                $fix = $phpcsFile->addFixableError('Trailing comma required after last attribute in a multi-line, multi-attribute attribute block', $beforeCloser, 'RequiredMultiAttributeMultiLine');
                if ($fix === \true) {
        ...
        if ($tokens[$beforeCloser]['code'] === \T_COMMA) {
            ...
            $fix = $phpcsFile->addFixableError('Trailing comma is not allowed when a multi-line attribute block only contains a single attribute', $beforeCloser, 'ForbiddenSingleAttributeMultiLine');
            if ($fix === \true) {
                $phpcsFile->fixer->replaceToken($beforeCloser, '');
            }
        } else {
