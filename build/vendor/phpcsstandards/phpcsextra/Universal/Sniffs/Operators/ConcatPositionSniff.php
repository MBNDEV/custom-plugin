...
final class ConcatPositionSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $fix = $phpcsFile->addFixableError('The concatenation operator for multi-line concatenations should always be at the %s of a line.', $stackPtr, 'Incorrect', [$this->allowOnly]);
        if ($fix === \true) {
            if ($this->allowOnly === self::POSITION_END) {
                ...
                if ($position === self::POSITION_START && $tokens[$stackPtr + 1]['code'] === \T_WHITESPACE) {
            ...
            if ($position === self::POSITION_END && $tokens[$stackPtr - 1]['code'] === \T_WHITESPACE) {
                // Remove trailing space.
                $phpcsFile->fixer->replaceToken($stackPtr - 1, '');
            } elseif ($position === self::POSITION_STANDALONE) {
                // Remove potential indentation space.
