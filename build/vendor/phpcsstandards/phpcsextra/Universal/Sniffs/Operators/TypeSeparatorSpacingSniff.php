 ...
 */
namespace _PhpScoperdd275a4cd119\PHPCSExtra\Universal\Sniffs\Operators;
...
final class TypeSeparatorSpacingSniff implements Sniff
{
     ...
     */
    private $targetTokens = [\T_TYPE_UNION => \T_TYPE_UNION, \T_TYPE_INTERSECTION => \T_TYPE_INTERSECTION, \T_TYPE_OPEN_PARENTHESIS => \T_TYPE_OPEN_PARENTHESIS, \T_TYPE_CLOSE_PARENTHESIS => \T_TYPE_CLOSE_PARENTHESIS];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_TYPE_OPEN_PARENTHESIS) {
            if ($tokens[$prevNonEmpty]['code'] === \T_COLON || $tokens[$prevNonEmpty]['code'] === \T_CONST || isset(Collections::propertyModifierKeywords()[$tokens[$prevNonEmpty]['code']]) === \true) {
                // Start of return type or property/const type. Always demand 1 space.
                $expectedSpaces = 1;
            }
            if ($tokens[$prevNonEmpty]['code'] === \T_OPEN_PARENTHESIS || $tokens[$prevNonEmpty]['code'] === \T_COMMA) {
        ...
        if (isset($this->targetTokens[$tokens[$prevNonEmpty]['code']]) === \true) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_TYPE_CLOSE_PARENTHESIS) {
            if ($tokens[$nextNonEmpty]['code'] === \T_OPEN_CURLY_BRACKET || $tokens[$nextNonEmpty]['code'] === \T_VARIABLE || $tokens[$nextNonEmpty]['code'] === \T_STRING) {
