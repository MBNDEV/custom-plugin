 ...
 */
namespace _PhpScoperdd275a4cd119\PHPCSExtra\Universal\Sniffs\PHP;
...
final class NoFQNTrueFalseNullSniff implements Sniff
{
    ...
    {
        $targets = [\T_TRUE, \T_FALSE, \T_NULL];
        if (\version_compare(Config::VERSION, '4.0.0', '>=') === \true) {
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $fix = $phpcsFile->addFixableError('The special PHP constant "%s" should not be fully qualified.', $stackPtr, 'Found', [$contentLC]);
        if ($fix === \true) {
            if ($contentLC === '\true' || $contentLC === '\false' || $contentLC === '\null') {
                // PHPCS 4.x.
