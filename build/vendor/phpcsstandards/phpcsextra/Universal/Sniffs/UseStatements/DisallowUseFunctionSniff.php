    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($statements['function'] as $alias => $qualifiedName) {
            ...
            if ($reportPtr === \false) {
                ...
                $reportPtr = $phpcsFile->findNext([\T_NAME_QUALIFIED], $stackPtr + 1, $endOfStatement, \false, $qualifiedName);
                if ($reportPtr === \false) {
                    // This may be an imported function (incorrectly) passed as an FQN.
                    $reportPtr = $phpcsFile->findNext([\T_NAME_FULLY_QUALIFIED], $stackPtr + 1, $endOfStatement, \false, '\\' . $qualifiedName);
                    if ($reportPtr === \false) {
                        ...
                        for ($i = $groupStart + 1; $i < $endOfStatement; $i++) {
                            if ($tokens[$i]['code'] === \T_NAME_QUALIFIED || $tokens[$i]['code'] === \T_NAME_FULLY_QUALIFIED) {
            ...
            } elseif ($this->currentNamespace !== '' && \stripos($qualifiedName, $this->currentNamespace . '\\') === 0) {
                $sameNamespace = \true;
