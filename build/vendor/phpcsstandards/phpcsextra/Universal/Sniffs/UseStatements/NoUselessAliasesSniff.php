 ...
 */
namespace _PhpScoperdd275a4cd119\PHPCSExtra\Universal\Sniffs\UseStatements;
...
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\NamingConventions;
            ...
            foreach ($statements as $alias => $qualifiedName) {
                ...
                foreach ($aliasPtrs as $asPtr => $aliasPtr) {
                    ...
                    if (isset(Collections::nameTokens()[$tokens[$prev]['code']]) === \false) {
                        ...
                        continue;
                        // @codeCoverageIgnore
                    } elseif ($tokens[$prev]['code'] === \T_STRING && $tokens[$prev]['content'] !== $unqualifiedName) {
                        continue;
                    } elseif ($tokens[$prev]['code'] === \T_NAME_QUALIFIED && $tokens[$prev]['content'] !== $qualifiedName && \substr($qualifiedName, -\strlen($tokens[$prev]['content'])) !== $tokens[$prev]['content']) {
                        continue;
                    } elseif ($tokens[$prev]['code'] === \T_NAME_FULLY_QUALIFIED && $tokens[$prev]['content'] !== '\\' . $qualifiedName && \substr($qualifiedName, -\strlen($tokens[$prev]['content'])) !== $tokens[$prev]['content']) {
                        continue;
                    }
