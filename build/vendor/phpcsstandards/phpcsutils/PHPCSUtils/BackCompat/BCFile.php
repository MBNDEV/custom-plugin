 ...
 */
namespace _PhpScoperdd275a4cd119\PHPCSUtils\BackCompat;
...
final class BCFile
{
    ...
    public static function getMethodParameters(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $paramStart; $i <= $closer; $i++) {
            ...
            // end of it.
            if ($tokens[$i]['code'] !== T_TYPE_OPEN_PARENTHESIS && isset($tokens[$i]['parenthesis_opener']) === \true) {
    ...
    public static function getMemberProperties(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$ptr]) === \false || isset(Tokens::$ooScopeTokens[$tokens[$ptr]['code']]) === \false || $tokens[$ptr]['code'] === \T_ENUM) {
            throw new RuntimeException('$stackPtr is not a class member var');
        }
        ...
        if (empty($tokens[$stackPtr]['nested_parenthesis']) === \false) {
            ...
            if ($deepestOpen > $ptr && isset($tokens[$deepestOpen]['parenthesis_owner']) === \true && $tokens[$tokens[$deepestOpen]['parenthesis_owner']]['code'] === \T_FUNCTION) {
                throw new RuntimeException('$stackPtr is not a class member var');
            }
        }
        ...
        if ($i < $stackPtr) {
            ...
            for ($i; $i < $stackPtr; $i++) {
                ...
                if (isset($valid[$tokens[$i]['code']]) === \true) {
                    $typeEndToken = $i;
                    if ($typeToken === \false) {
                        $typeToken = $i;
                    }
    ...
    public static function findExtendedClassName(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] !== \T_CLASS && $tokens[$stackPtr]['code'] !== T_ANON_CLASS && $tokens[$stackPtr]['code'] !== \T_INTERFACE) {
            return \false;
        }
        ...
        $find[] = \T_WHITESPACE;
        $end = $phpcsFile->findNext($find, $extendsIndex + 1, $classOpenerIndex + 1, \true);
    ...
    public static function findImplementedInterfaceNames(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] !== \T_CLASS && $tokens[$stackPtr]['code'] !== T_ANON_CLASS && $tokens[$stackPtr]['code'] !== \T_ENUM) {
            return \false;
        }
