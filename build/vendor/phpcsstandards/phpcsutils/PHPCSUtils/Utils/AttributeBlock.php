...
final class AttributeBlock
{
    ...
    public static function appliesTo(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] !== \T_ATTRIBUTE && $tokens[$stackPtr]['code'] !== \T_ATTRIBUTE_END && Context::inAttribute($phpcsFile, $stackPtr) === \false) {
        ...
        if (isset($allowedTokens) === \false) {
            ...
            $allowedTokens += [\T_BITWISE_AND => \T_BITWISE_AND, \T_ELLIPSIS => \T_ELLIPSIS];
        }
        for ($i = $tokens[$stackPtr]['attribute_closer'] + 1; $i <= $phpcsFile->numTokens; $i++) {
            ...
            if (isset(Tokens::$ooScopeTokens[$tokens[$i]['code']]) === \true || isset(Collections::functionDeclarationTokens()[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === \T_CONST || $tokens[$i]['code'] === \T_VARIABLE) {
                $attributeTarget = $i;
            }
            break;
        }
    ...
    public static function getAttributes(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $opener + 1; $i <= $closer; $i++) {
            ...
            if ($tokens[$i]['code'] === \T_COMMA || $i === $closer) {
                // We've reached the end of the name.
                if ($currentName === '') {
                    // Parse error. Stop parsing this attribute block.
                    break;
                }
                $attributes[] = ['name' => $currentName, 'name_token' => $nameToken, 'start' => $start, 'end' => $i - 1, 'comma_token' => $tokens[$i]['code'] === \T_COMMA ? $i : \false];
                if ($i === $closer) {
                    break;
                }
                ...
                $hasNext = $phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, $closer, \true);
                if ($hasNext === \false) {
                    break;
                }
