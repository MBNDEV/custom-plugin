...
use _PhpScoperdd275a4cd119\PHPCSUtils\Exceptions\OutOfBoundsStackPtr;
use _PhpScoperdd275a4cd119\PHPCSUtils\Exceptions\TypeError;
use _PhpScoperdd275a4cd119\PHPCSUtils\Exceptions\UnexpectedTokenType;
use _PhpScoperdd275a4cd119\PHPCSUtils\Exceptions\ValueError;
use _PhpScoperdd275a4cd119\PHPCSUtils\Internal\AttributeHelper;
use _PhpScoperdd275a4cd119\PHPCSUtils\Internal\Cache;
...
final class Constants
{
    ...
    public static function getProperties(File $phpcsFile, $stackPtr)
    {
        ...
        $namePtr = $phpcsFile->findPrevious(Tokens::$emptyTokens, $assignmentPtr - 1, $stackPtr + 1, \true);
        $returnValue = ['scope' => 'public', 'scope_token' => \false, 'is_final' => \false, 'final_token' => \false, 'type' => '', 'type_token' => \false, 'type_end_token' => \false, 'nullable_type' => \false, 'name_token' => $namePtr, 'equal_token' => $assignmentPtr];
        for ($i = $stackPtr - 1;; $i--) {
            // Skip over potentially large docblocks.
            if ($tokens[$i]['code'] === \T_DOC_COMMENT_CLOSE_TAG && isset($tokens[$i]['comment_opener'])) {
        ...
        for ($i = $stackPtr + 1; $i < $namePtr; $i++) {
            ...
            if ($tokens[$i]['code'] === \T_NULLABLE) {
                $returnValue['nullable_type'] = \true;
                continue;
            }
            if (isset($constantTypeTokens[$tokens[$i]['code']]) === \true) {
                $typeEndToken = $i;
                if ($typeToken === \false) {
                    $typeToken = $i;
                }
        ...
        if ($type !== '' && $returnValue['nullable_type'] === \true) {
            $type = '?' . $type;
        }
