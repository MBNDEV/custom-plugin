 ...
 */
namespace _PhpScoperdd275a4cd119\PHPCSUtils\Utils;

use _PhpScoperdd275a4cd119\PHPCSUtils\Exceptions\TypeError;
/**
...
final class TypeString
{
     ...
     */
    private static $keywordTypes = ['array' => 'array', 'bool' => 'bool', 'callable' => 'callable', 'false' => 'false', 'float' => 'float', 'int' => 'int', 'iterable' => 'iterable', 'mixed' => 'mixed', 'never' => 'never', 'null' => 'null', 'object' => 'object', 'parent' => 'parent', 'self' => 'self', 'static' => 'static', 'string' => 'string', 'true' => 'true', 'void' => 'void'];
    /**
    ...
    public static function isSingular($typeString)
    {
        ...
        return empty($typeString) === \false && \strpos($typeString, '?') === \false && \strpos($typeString, '|') === \false && \strpos($typeString, '&') === \false && \strpos($typeString, '(') === \false && \strpos($typeString, ')') === \false;
    }
    /**
    ...
    public static function isNullable($typeString)
    {
        ...
        $matched = \preg_match('`(?<before>^|[^|&(?\s]+\s*\|)\s*[\\\\]?null\s*(?<after>\|\s*[^|&)?\s]+|$)`i', $typeString, $matches);
        return $matched === 1 && (empty($matches['before']) === \false || empty($matches['after']) === \false);
    }
    /**
    ...
    {
        return \is_string($typeString) && \strpos($typeString, '?') === \false && \strpos($typeString, '|') !== \false && \strpos($typeString, '&') === \false && \strpos($typeString, '(') === \false && \strpos($typeString, ')') === \false && \preg_match('`^[^|&()?\s]+(\s*\|\s*[^|&()?\s]+)+$`', $typeString) === 1;
    }
    /**
    ...
    {
        return \is_string($typeString) && \strpos($typeString, '?') === \false && \strpos($typeString, '|') !== \false && \strpos($typeString, '&') !== \false && \strpos($typeString, '(') !== \false && \strpos($typeString, ')') !== \false && \preg_match(self::INVALID_DNF_REGEX, $typeString) !== 1;
    }
    /**
     ...
     */
    public static function toArray($typeString, $normalize = \true)
    {
        if (\is_string($typeString) === \false) {
     ...
     * @throws \PHPCSUtils\Exceptions\TypeError If passed $typeString is not a string.
     */
    public static function toArrayUnique($typeString, $normalize = \true)
    {
    ...
    public static function filterOOTypes(array $types)
    {
        return \array_filter($types, static function ($type) {
            return \is_string($type) === \true && self::isKeyword($type) === \false;
        });
    }
}
