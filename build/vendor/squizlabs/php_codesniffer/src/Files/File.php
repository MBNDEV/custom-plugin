 ...
 */
namespace _PhpScoperdd275a4cd119\PHP_CodeSniffer\Files;

use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Config;
use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Exceptions\RuntimeException;
use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Exceptions\TokenizerException;
use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Fixer;
...
class File
{
        ...
        foreach ($this->tokens as $stackPtr => $token) {
            ...
            if ($checkAnnotations === \true && ($token['code'] === \T_COMMENT || $token['code'] === T_PHPCS_IGNORE_FILE || $token['code'] === T_PHPCS_SET || $token['code'] === T_DOC_COMMENT_STRING || $token['code'] === T_DOC_COMMENT_TAG || $inTests === \true && $token['code'] === \T_INLINE_HTML)) {
                ...
                if (strpos($commentText, '@codingStandards') !== \false) {
                    ...
                    //end if
                } else if (substr($commentTextLower, 0, 16) === 'phpcs:ignorefile' || substr($commentTextLower, 0, 17) === '@phpcs:ignorefile') {
                    // Ignoring the whole file, just a little late.
                    $this->errors = [];
                    $this->warnings = [];
                    $this->errorCount = 0;
                    ...
                    $this->fixableCount = 0;
                    return;
                } else if (substr($commentTextLower, 0, 9) === 'phpcs:set' || substr($commentTextLower, 0, 10) === '@phpcs:set') {
        ...
        if (trim($this->path, '\'"') === 'STDIN') {
            ...
            foreach ($checkCodes as $checkCode) {
                ...
                if (isset($this->configCache['includePatterns'][$checkCode]) === \true) {
                    ...
                    $excluding = \false;
                } else if (isset($this->configCache['ignorePatterns'][$checkCode]) === \true) {
    ...
    public function getDeclarationName($stackPtr)
    {
        ...
        } else if (isset($this->tokens[$stackPtr]['scope_opener']) === \true) {
    ...
    public function findPrevious($types, $start, $end = null, $exclude = \false, $value = null, $local = \false)
    {
        ...
        for ($i = $start; $i >= $end; $i--) {
            ...
            if ($local === \true) {
                ...
                } else if (isset($this->tokens[$i]['bracket_opener']) === \true && $i === $this->tokens[$i]['bracket_closer']) {
                    $i = $this->tokens[$i]['bracket_opener'];
                } else if (isset($this->tokens[$i]['parenthesis_opener']) === \true && $i === $this->tokens[$i]['parenthesis_closer']) {
    ...
    public function findEndOfStatement($start, $ignore = null)
    {
        ...
        for ($i = $start; $i < $this->numTokens; $i++) {
            ...
            } else if (isset($this->tokens[$i]['bracket_closer']) === \true && $i === $this->tokens[$i]['bracket_opener']) {
                $i = $this->tokens[$i]['bracket_closer'];
            } else if (isset($this->tokens[$i]['parenthesis_closer']) === \true && $i === $this->tokens[$i]['parenthesis_opener']) {
                $i = $this->tokens[$i]['parenthesis_closer'];
