 ...
 */
namespace _PhpScoperdd275a4cd119\PHP_CodeSniffer\Standards\PSR2\Sniffs\Classes;

use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Files\File;
use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Standards\PEAR\Sniffs\Classes\ClassDeclarationSniff as PEARClassDeclarationSniff;
...
class ClassDeclarationSniff extends PEARClassDeclarationSniff
{
    ...
    public function processOpen(File $phpcsFile, $stackPtr)
    {
        ...
        if ($className !== null) {
            $start = $className;
        } else if (isset($tokens[$stackPtr]['parenthesis_closer']) === \true) {
        ...
        foreach ($classNames as $n => $className) {
            ...
            if ($checkingImplements === \true && $multiLineImplements === \true && ($tokens[$className - 1]['code'] !== \T_NS_SEPARATOR || $tokens[$className - 2]['code'] !== \T_STRING && $tokens[$className - 2]['code'] !== \T_NAMESPACE)) {
                ...
                } else if (isset(Tokens::$commentTokens[$tokens[$prev]['code']]) === \false && $tokens[$prev]['line'] !== $tokens[$className]['line'] - 1 || $tokens[$prev]['line'] === $tokens[$className]['line']) {
                    if ($keywordTokenType === \T_EXTENDS) {
                ...
                //end if
            } else if ($tokens[$className - 1]['code'] !== \T_NS_SEPARATOR || $tokens[$className - 2]['code'] !== \T_STRING && $tokens[$className - 2]['code'] !== \T_NAMESPACE) {
