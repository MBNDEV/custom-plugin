 ...
 */
namespace _PhpScoperdd275a4cd119\PHP_CodeSniffer\Standards\Squiz\Sniffs\PHP;
...
class EmbeddedPhpSniff implements Sniff
{
    ...
    private function validateMultilineEmbeddedPhp($phpcsFile, $stackPtr, $closingTag)
    {
        ...
        if ($tokens[$lastContent]['line'] === $tokens[$closingTag]['line']) {
            ...
            //end if
        } else if ($firstContentAfterBlock !== \false && $tokens[$firstContentAfterBlock]['line'] === $tokens[$closingTag]['line']) {
            $error = 'Closing PHP tag must be on a line by itself';
    ...
    private function validateInlineEmbeddedPhp($phpcsFile, $stackPtr, $closeTag)
    {
        ...
        } else if (($tokens[$closeTag - 1]['code'] === \T_COMMENT || isset(Tokens::$phpcsCommentTokens[$tokens[$closeTag - 1]['code']]) === \true) && substr($tokens[$closeTag - 1]['content'], -1) === ' ') {
        ...
        if ($trailingSpace !== 1) {
            ...
            if ($fix === \true) {
                ...
                } else if ($tokens[$closeTag - 1]['code'] === \T_COMMENT || isset(Tokens::$phpcsCommentTokens[$tokens[$closeTag - 1]['code']]) === \true) {
    ...
    private function calculateLineIndent(File $phpcsFile, $stackPtr)
    {
        ...
        } else if ($tokens[$firstOnLine]['code'] === \T_INLINE_HTML || $tokens[$firstOnLine]['code'] === \T_END_HEREDOC || $tokens[$firstOnLine]['code'] === T_END_NOWDOC) {
