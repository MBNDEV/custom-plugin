...
use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Util;
    ...
    public function tokenize($string)
    {
        ...
        for ($i = 0; $i < $numChars; $i++) {
            ...
            if (isset($this->tokenValues[strtolower($buffer)]) === \true && (preg_match('|[a-zA-z0-9_]|', $char) === 0 || isset($chars[$i + 1]) === \false || preg_match('|[a-zA-z0-9_]|', $chars[$i + 1]) === 0)) {
                ...
                //end if
            } else if (isset($this->tokenValues[strtolower($char)]) === \true) {
            ...
            if ($inComment === '' && array_key_exists($buffer, $this->commentTokens) === \true) {
                ...
                } else if ($this->commentTokens[$inComment] === $buffer) {
                    $inComment = '';
                }
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    if ($inComment === '') {
    ...
    public function processAdditional()
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if ($this->tokens[$i]['code'] === \T_FUNCTION && isset($this->tokens[$i]['scope_opener']) === \true) {
                ...
                //end if
                continue;
            } else if ($this->tokens[$i]['code'] === T_OPEN_CURLY_BRACKET && isset($this->tokens[$i]['scope_condition']) === \false && isset($this->tokens[$i]['bracket_closer']) === \true) {
