 ...
 */
namespace _PhpScoperdd275a4cd119\PHP_CodeSniffer\Tokenizers;
...
class PHP extends Tokenizer
{
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            */
            if (\PHP_VERSION_ID < 70000 && $tokenIsArray === \true && $token[0] === \T_YIELD && isset($tokens[$stackPtr + 1]) === \true && isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE && strpos($tokens[$stackPtr + 1][1], $this->eolChar) === \false && $tokens[$stackPtr + 2][0] === \T_STRING && strtolower($tokens[$stackPtr + 2][1]) === 'from') {
                ...
                $stackPtr += 2;
                continue;
            } else if (\PHP_VERSION_ID < 80300 && $tokenIsArray === \true && $token[0] === \T_STRING && strtolower($token[1]) === 'from' && $finalTokens[$lastNotEmptyToken]['code'] === \T_YIELD) {
                /*
                ...
                */
                ...
                continue;
            } else if (\PHP_VERSION_ID >= 70000 && $tokenIsArray === \true && $token[0] === \T_YIELD_FROM && strpos($token[1], $this->eolChar) !== \false && preg_match('`^yield\s+from$`i', $token[1]) === 1) {
                /*
                ...
                */
                ...
                continue;
            } else if (\PHP_VERSION_ID >= 80300 && $tokenIsArray === \true && $token[0] === \T_YIELD_FROM && preg_match('`^yield[ \t]+from$`i', $token[1]) !== 1 && stripos($token[1], 'yield') === 0) {
                /*
            ...
            if ($tokenIsArray === \false && $token[0] === '?') {
                ...
                for ($i = $stackPtr - 1; $i >= 0; $i--) {
                    ...
                    if ($tokenType === \T_FUNCTION || $tokenType === \T_FN || isset(Tokens::$methodPrefixes[$tokenType]) === \true || isset(Tokens::$scopeModifiers[$tokenType]) === \true || $tokenType === \T_VAR || $tokenType === \T_READONLY) {
                        ...
                        $newToken['type'] = 'T_NULLABLE';
                        break;
                    } else if (in_array($tokenType, [\T_DOUBLE_ARROW, \T_OPEN_TAG, \T_OPEN_TAG_WITH_ECHO, '=', '{', ';'], \true) === \true) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            echo "\t\t* token {$stackPtr} changed from ? to T_INLINE_THEN" . \PHP_EOL;
                        }
                        $newToken['code'] = T_INLINE_THEN;
            ...
            */
            if ($tokenIsArray === \true && strpos($token[1], $this->eolChar) !== \false) {
                ...
                if ($tokenIsArray === \true && $token[0] === \T_STRING) {
                    ...
                    } else if (isset($this->tstringContexts[$finalTokens[$lastNotEmptyToken]['code']]) === \true && $finalTokens[$lastNotEmptyToken]['code'] !== \T_CONST) {
                        $preserveTstring = \true;
        ...
        for ($i = $numTokens - 1; $i >= 0; $i--) {
            ...
            if ($this->tokens[$i]['code'] === \T_FUNCTION) {
                ...
                */
                ...
                //end if
                continue;
            } else if ($this->tokens[$i]['code'] === \T_CLASS && isset($this->tokens[$i]['scope_opener']) === \true) {
                /*
                ...
                */
                ...
                //end if
                continue;
            } else if ($this->tokens[$i]['code'] === \T_FN && isset($this->tokens[$i + 1]) === \true) {
                ...
                //end if
                continue;
            } else if ($this->tokens[$i]['code'] === T_BITWISE_OR || $this->tokens[$i]['code'] === T_BITWISE_AND || $this->tokens[$i]['code'] === T_CLOSE_PARENTHESIS) {
                ...
                */
                ...
                continue;
            } else if ($this->tokens[$i]['code'] === T_TRUE || $this->tokens[$i]['code'] === T_FALSE || $this->tokens[$i]['code'] === T_NULL || $this->tokens[$i]['code'] === \T_EXIT) {
                for ($x = $i + 1; $x < $numTokens; $x++) {
                    if (isset(Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \false) {
                        // Non-whitespace content.
                        break;
    ...
    private function createAttributesNestingMap()
    {
        ...
        for ($i = 0; $i < $this->numTokens; $i++) {
            ...
            } else if (isset($this->tokens[$i]['attribute_closer']) === \true && $i === $this->tokens[$i]['attribute_closer']) {
                array_pop($map);
                if (empty($map) === \false) {
                    $this->tokens[$i]['nested_attributes'] = $map;
                }
