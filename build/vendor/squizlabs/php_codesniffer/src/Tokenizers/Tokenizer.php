...
use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Exceptions\TokenizerException;
...
abstract class Tokenizer
{
    ...
    private function createPositionMap()
    {
        ...
        for ($i = 0; $i < $this->numTokens; $i++) {
            ...
            if (isset($this->knownLengths[$this->tokens[$i]['code']]) === \true) {
                ...
                $currColumn += $length;
            } else if ($tabWidth === 0 || isset($tokensWithTabs[$this->tokens[$i]['code']]) === \false || strpos($this->tokens[$i]['content'], "\t") === \false) {
            ...
            if ($this->tokens[$i]['code'] === \T_COMMENT || $this->tokens[$i]['code'] === T_DOC_COMMENT_STRING || $this->tokens[$i]['code'] === T_DOC_COMMENT_TAG || $inTests === \true && $this->tokens[$i]['code'] === \T_INLINE_HTML) {
                ...
                if (strpos($commentText, '@codingStandards') !== \false) {
                    ...
                    if ($ignoring === null && strpos($commentText, '@codingStandardsIgnoreStart') !== \false) {
                    ...
                    } else if ($ignoring !== null && strpos($commentText, '@codingStandardsIgnoreEnd') !== \false) {
                        if ($ownLine === \true) {
                        ...
                        $ignoring = null;
                    } else if ($ignoring === null && strpos($commentText, '@codingStandardsIgnoreLine') !== \false) {
                        $ignoring = ['.all' => \true];
                        if ($ownLine === \true) {
                            $this->ignoredLines[$this->tokens[$i]['line']] = $ignoring;
                    ...
                    //end if
                } else if (substr($commentTextLower, 0, 6) === 'phpcs:' || substr($commentTextLower, 0, 7) === '@phpcs:') {
                    ...
                    if (substr($commentTextLower, 0, 9) === 'phpcs:set') {
                        ...
                        if ($ignoring !== null) {
                            ...
                            if (empty($additionalText) === \true) {
                                ...
                                if (empty($ignoring) === \true) {
                                    $ignoring = null;
                                } else if (isset($ignoring['.except']) === \true) {
                        ...
                        if (empty($additionalText) === \true) {
                            ...
                            $parts = explode(',', $additionalText);
                            foreach ($parts as $sniffCode) {
                                $ignoreRules[trim($sniffCode)] = \true;
                            }
                        }
    ...
    private function createParenthesisNestingMap()
    {
        ...
        for ($i = 0; $i < $this->numTokens; $i++) {
            ...
            } else if (isset($this->tokens[$i]['parenthesis_closer']) === \true && $i === $this->tokens[$i]['parenthesis_closer']) {
                array_pop($map);
                if (empty($map) === \false) {
                    $this->tokens[$i]['nested_parenthesis'] = $map;
                }
        ...
        for ($i = $stackPtr + 1; $i < $this->numTokens; $i++) {
            ...
            if ($opener !== null && (isset($this->tokens[$i]['scope_opener']) === \false || $this->scopeOpeners[$this->tokens[$stackPtr]['code']]['shared'] === \true) && isset($this->scopeOpeners[$currType]['end'][$tokenType]) === \true) {
                if ($ignore > 0 && $tokenType === T_CLOSE_CURLY_BRACKET) {
                    ...
                    $ignore--;
                    continue;
                } else if ($this->tokens[$opener]['code'] === T_OPEN_CURLY_BRACKET && $tokenType !== T_CLOSE_CURLY_BRACKET) {
                    ...
                    if ($validCloser === \true) {
                        ...
                        if ($this->scopeOpeners[$this->tokens[$stackPtr]['code']]['shared'] === \true) {
                            ...
                            return $opener;
                        } else if ($scopeCloser === $i && isset($this->scopeOpeners[$tokenType]) === \true) {
            ...
            if (isset($this->scopeOpeners[$currType]['start'][$tokenType]) === \true && $opener === null) {
                ...
                //end if
            } else if ($tokenType === T_SEMICOLON && $opener === null && (isset($this->tokens[$stackPtr]['parenthesis_closer']) === \false || $i > $this->tokens[$stackPtr]['parenthesis_closer'])) {
                ...
                $ignore--;
            } else if ($opener === null && isset($this->scopeOpeners[$currType]) === \true) {
            ...
            } else if ($opener !== null && $tokenType !== \T_BREAK && isset($this->endScopeTokens[$tokenType]) === \true) {
