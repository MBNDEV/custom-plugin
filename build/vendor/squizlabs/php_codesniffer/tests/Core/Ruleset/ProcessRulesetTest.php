...
final class ProcessRulesetTest extends TestCase
{
    ...
    public function testSniffsFileNotDirectory()
    {
        ...
        $expected = ['Generic.PHP.BacktickOperator' => '_PhpScoperdd275a4cd119\PHP_CodeSniffer\Standards\Generic\Sniffs\PHP\BacktickOperatorSniff'];
    ...
    public function testAutoExpandSniffsDirectory()
    {
        ...
        $ruleset = new Ruleset($config);
        $std = 'TestStandard';
        $sniffDir = '_PhpScoperdd275a4cd119\Fixtures\TestStandard\Sniffs';
    ...
    public function testExcludeSniffGroup()
    {
        ...
        $config = new ConfigDouble(["--standard={$standard}"]);
        $ruleset = new Ruleset($config);
        $expected = ['PSR1.Classes.ClassDeclaration' => '_PhpScoperdd275a4cd119\PHP_CodeSniffer\Standards\PSR1\Sniffs\Classes\ClassDeclarationSniff', 'PSR1.Methods.CamelCapsMethodName' => '_PhpScoperdd275a4cd119\PHP_CodeSniffer\Standards\PSR1\Sniffs\Methods\CamelCapsMethodNameSniff'];
        // Sort the value to make the tests stable as different OSes will read directories
        // in a different order and the order is not relevant for these tests. Just the values.
        $actual = $ruleset->sniffCodes;
        ksort($actual);
