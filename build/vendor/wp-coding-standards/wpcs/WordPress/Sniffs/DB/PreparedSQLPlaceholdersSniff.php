 ...
 */
namespace _PhpScoperdd275a4cd119\WordPressCS\WordPress\Sniffs\DB;
...
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\WPDBTrait;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Sniff;
/**
...
final class PreparedSQLPlaceholdersSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        for ($i = $query['start']; $i <= $query['end']; $i++) {
            ...
            if (!isset(Tokens::$textStringTokens[$this->tokens[$i]['code']])) {
                ...
                if (\T_STRING === $this->tokens[$i]['code']) {
                    if ('sprintf' === strtolower($this->tokens[$i]['content'])) {
                        ...
                        $ignore_tokens = Tokens::$emptyTokens + array(\T_STRING_CONCAT => \T_STRING_CONCAT, \T_NS_SEPARATOR => \T_NS_SEPARATOR);
                        $prev = $this->phpcsFile->findPrevious($ignore_tokens, $i - 1, $query['start'], \true);
                        if (isset(Tokens::$textStringTokens[$this->tokens[$prev]['code']])) {
                            ...
                            if (preg_match('`\s+IN\s*\(\s*(' . $regex_quote . ')?$`i', $prev_content, $match) > 0) {
                                if (isset($match[1]) && $regex_quote !== $this->regex_quote) {
                                    $this->phpcsFile->addError('Dynamic placeholder generation should not have surrounding quotes.', $prev, 'QuotedDynamicPlaceholderGeneration');
                                }
            ...
            if (\T_DOUBLE_QUOTED_STRING === $this->tokens[$i]['code'] || \T_HEREDOC === $this->tokens[$i]['code']) {
                ...
                if ($stripped_content !== $content) {
                    $vars_without_wpdb = array_filter(TextStrings::getEmbeds($content), static function ($symbol) {
            ...
            if ($this->wp_version_compare($this->minimum_wp_version, '6.2', '<')) {
                if (preg_match_all('`' . self::PREPARE_PLACEHOLDER_REGEX . '`x', $content, $matches) > 0) {
                    if (!empty($matches[0])) {
                        foreach ($matches[0] as $match) {
                            if ('i' === substr($match, -1)) {
                                $this->phpcsFile->addError('The %%i modifier is only supported in WP 6.2 or higher. Found: "%s".', $i, 'UnsupportedIdentifierPlaceholder', array($match));
                            }
                        }
                    }
                }
            ...
            if (preg_match_all($regex, $content, $matches) > 0) {
                if (!empty($matches)) {
                    foreach ($matches['placeholder'] as $index => $match) {
                        if ('i' === substr($match, -1)) {
                            $this->phpcsFile->addError('Placeholders used for identifiers (%%i) in the query string in $wpdb->prepare() are always quoted automagically. Please remove the surrounding quotes. Found: %s', $i, 'QuotedIdentifierPlaceholder', array($matches[0][$index]));
                        }
                    }
                }
            ...
            if (preg_match_all($regex, $content, $matches) > 0) {
                if (!empty($matches[0])) {
                    foreach ($matches[0] as $match) {
                        if (substr($match, -1) !== 'i' && preg_match('`^%[dfFsi]$`', $match) !== 1) {
                            // Identifiers must always be unquoted.
        ...
        unset($replacements['query'], $replacements[1]);
        // Remove the query param, whether passed positionally or named.
        ...
        if (\false !== $args_param && 2 === $total_parameters) {
            $next = $this->phpcsFile->findNext(Tokens::$emptyTokens, $args_param['start'], $args_param['end'] + 1, \true);
            if (\false !== $next && (\T_ARRAY === $this->tokens[$next]['code'] || isset(Collections::shortArrayListOpenTokensBC()[$this->tokens[$next]['code']]) && Arrays::isShortArray($this->phpcsFile, $next) === \true)) {
    ...
    protected function analyse_sprintf($sprintf_params)
    {
        ...
        // Remove the positionally passed $format param.
        foreach ($sprintf_params as $sprintf_param) {
            $implode = $this->phpcsFile->findNext(Tokens::$emptyTokens + array(\T_NS_SEPARATOR => \T_NS_SEPARATOR), $sprintf_param['start'], $sprintf_param['end'], \true);
    ...
    protected function analyse_implode($implode_token)
    {
        ...
        if (\false === $implode_separator_param || preg_match('`^(["\']), ?\1$`', $implode_separator_param['clean']) !== 1) {
            return \false;
        }
        ...
        $array_fill = $this->phpcsFile->findNext(Tokens::$emptyTokens + array(\T_NS_SEPARATOR => \T_NS_SEPARATOR), $implode_array_param['start'], $implode_array_param['end'], \true);
        ...
        if ("'%i'" === $array_fill_value_param['clean'] || '"%i"' === $array_fill_value_param['clean']) {
            $firstNonEmpty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $array_fill_value_param['start'], $array_fill_value_param['end'], \true);
            $this->phpcsFile->addError('The %i placeholder cannot be used within SQL `IN()` clauses.', $firstNonEmpty, 'IdentifierWithinIN');
            return \false;
        }
