 ...
 */
namespace _PhpScoperdd275a4cd119\WordPressCS\WordPress\Sniffs\Security;

use _PhpScoperdd275a4cd119\PHP_CodeSniffer\Util\Tokens;
use _PhpScoperdd275a4cd119\PHPCSUtils\BackCompat\BCFile;
use _PhpScoperdd275a4cd119\PHPCSUtils\Tokens\Collections;
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\Arrays;
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\Conditions;
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\Operators;
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\PassedParameters;
use _PhpScoperdd275a4cd119\PHPCSUtils\Utils\TextStrings;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\AbstractFunctionRestrictionsSniff;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\ArrayWalkingFunctionsHelper;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\ConstantsHelper;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\ContextHelper;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\EscapingFunctionsTrait;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\FormattingFunctionsHelper;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\PrintingFunctionsTrait;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\VariableHelper;
/**
...
class EscapeOutputSniff extends AbstractFunctionRestrictionsSniff
{
     ...
     */
    protected $unsafePrintingFunctions = array('_e' => array('alternative' => 'esc_html_e() or esc_attr_e()', 'params' => array(1 => 'text')), '_ex' => array('alternative' => 'echo esc_html_x() or echo esc_attr_x()', 'params' => array(1 => 'text')));
    /**
     ...
     */
    private $safe_components = array(\T_LNUMBER => \T_LNUMBER, \T_DNUMBER => \T_DNUMBER, \T_TRUE => \T_TRUE, \T_FALSE => \T_FALSE, \T_NULL => \T_NULL, \T_CONSTANT_ENCAPSED_STRING => \T_CONSTANT_ENCAPSED_STRING, \T_START_NOWDOC => \T_START_NOWDOC, \T_NOWDOC => \T_NOWDOC, \T_END_NOWDOC => \T_END_NOWDOC, \T_BOOLEAN_NOT => \T_BOOLEAN_NOT);
    /**
     ...
     * @var array<string|int, string|int>
     */
    private $target_keywords = array(\T_EXIT => \T_EXIT, \T_PRINT => \T_PRINT, \T_THROW => \T_THROW);
    /**
    ...
    public function getGroups()
    {
        ...
        return array('unsafe_printing_functions' => array('functions' => array_keys($this->unsafePrintingFunctions)), 'printing_functions' => array('functions' => array_keys($printing_functions)));
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        switch ($this->tokens[$stackPtr]['code']) {
            ...
            case \T_THROW:
                ...
                do {
                    $next_relevant = $this->phpcsFile->findNext($ignore, $next_relevant + 1, null, \true);
                ...
                if (\T_OPEN_PARENTHESIS !== $this->tokens[$next_relevant]['code'] || isset($this->tokens[$next_relevant]['parenthesis_closer']) === \false) {
                ...
                $call_token = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $next_relevant - 1, null, \true);
            ...
            case \T_PRINT:
                ...
                if (\T_COMMA !== $this->tokens[$end]['code'] && \T_SEMICOLON !== $this->tokens[$end]['code'] && \T_COLON !== $this->tokens[$end]['code'] && \T_DOUBLE_ARROW !== $this->tokens[$end]['code'] && isset($this->tokens[$end + 1])) {
                    /*
                ...
                if ($end >= $this->phpcsFile->numTokens - 1) {
                    $last_non_empty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $end, null, \true);
                ...
                if (\T_INLINE_THEN === $this->tokens[$prev_non_empty]['code']) {
                    ...
                    for ($i = $stackPtr + 1; $i < $end; $i++) {
                        ...
                        if (empty($this->tokens[$i]['nested_parenthesis']) && 0 === $target_nesting_level) {
                            $inline_else = $i;
                            break;
                        }
                        if (empty($this->tokens[$i]['nested_parenthesis']) === \false && \count($this->tokens[$i]['nested_parenthesis']) === $target_nesting_level) {
                            $inline_else = $i;
                            break;
                        }
                    }
    ...
    public function process_matched_token($stackPtr, $group_name, $matched_content)
    {
        ...
        if (\false === $next_non_empty || \T_OPEN_PARENTHESIS !== $this->tokens[$next_non_empty]['code'] || isset($this->tokens[$next_non_empty]['parenthesis_closer']) === \false) {
        ...
        if ('unsafe_printing_functions' === $group_name) {
            $error = $this->phpcsFile->addError("All output should be run through an escaping function (like %s), found '%s'.", $stackPtr, 'UnsafePrintingFunction', array($this->unsafePrintingFunctions[$matched_content]['alternative'], $matched_content));
        ...
        if ('_deprecated_file' === $matched_content) {
            ...
            if (\false !== $file_param) {
                ...
                if (preg_match('`^[\\\\]?basename\s*\(\s*__FILE__\s*\)$`', $file_param['clean']) === 1) {
                    unset($params[1], $params['file']);
                    // Remove the param, whether passed positionally or named.
                }
            }
            unset($file_param);
        }
    ...
    protected function check_code_is_escaped($start, $end, $code = 'OutputNotEscaped')
    {
         ...
         */
        $ternary = \false;
        $next_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $start + 1, null, \true);
        $last_non_empty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $end - 1, null, \true);
        if (\T_OPEN_PARENTHESIS !== $this->tokens[$next_non_empty]['code'] || \T_CLOSE_PARENTHESIS !== $this->tokens[$last_non_empty]['code'] || \T_OPEN_PARENTHESIS === $this->tokens[$next_non_empty]['code'] && \T_CLOSE_PARENTHESIS === $this->tokens[$last_non_empty]['code'] && isset($this->tokens[$next_non_empty]['parenthesis_closer']) && $this->tokens[$next_non_empty]['parenthesis_closer'] !== $last_non_empty) {
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if (isset(Tokens::$magicConstants[$this->tokens[$i]['code']]) || \T_NS_SEPARATOR === $this->tokens[$i]['code'] || \T_DOUBLE_ARROW === $this->tokens[$i]['code'] || \T_CLOSE_PARENTHESIS === $this->tokens[$i]['code']) {
                continue;
            }
            ...
            if (isset(Collections::arrayOpenTokensBC()[$this->tokens[$i]['code']])) {
                ...
                $array_items = PassedParameters::getParameters($this->phpcsFile, $i, 0, \true);
            ...
            if (\T_STRING === $this->tokens[$i]['code'] || \T_VARIABLE === $this->tokens[$i]['code'] || isset(Collections::ooHierarchyKeywords()[$this->tokens[$i]['code']]) || \T_NAMESPACE === $this->tokens[$i]['code']) {
                $skip_tokens = Tokens::$emptyTokens;
                $skip_tokens[\T_STRING] = \T_STRING;
                $skip_tokens[\T_NS_SEPARATOR] = \T_NS_SEPARATOR;
                $double_colon = $this->phpcsFile->findNext($skip_tokens, $i + 1, $end, \true);
                if (\false !== $double_colon && \T_DOUBLE_COLON === $this->tokens[$double_colon]['code']) {
                    $class_keyword = $this->phpcsFile->findNext(Tokens::$emptyTokens, $double_colon + 1, $end, \true);
                    if (\false !== $class_keyword && \T_STRING === $this->tokens[$class_keyword]['code'] && 'class' === strtolower($this->tokens[$class_keyword]['content'])) {
                        $i = $class_keyword;
                        continue;
                    }
                }
            ...
            if (isset(ContextHelper::get_safe_cast_tokens()[$this->tokens[$i]['code']])) {
                 ...
                 */
                $next_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, $end, \true);
                if (\false !== $next_non_empty && \T_MATCH === $this->tokens[$next_non_empty]['code'] && isset($this->tokens[$next_non_empty]['scope_closer'])) {
            ...
            if (\T_START_HEREDOC === $this->tokens[$i]['code']) {
                ...
                while (isset($this->tokens[$current]) && \T_HEREDOC === $this->tokens[$current]['code']) {
                    ...
                    if (!empty($embeds)) {
                        $this->phpcsFile->addError('All output should be run through an escaping function (see the Security sections in the WordPress Developer Handbooks), found interpolation in unescaped heredoc.', $current, 'HeredocOutputNotEscaped');
                    }
                    ++$current;
                }
                $i = $current;
            ...
            if (\T_STRING === $this->tokens[$i]['code']) {
                ...
                $function_opener = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
                $is_formatting_function = FormattingFunctionsHelper::is_formatting_function($functionName);
                if (\false !== $function_opener && \T_OPEN_PARENTHESIS === $this->tokens[$function_opener]['code']) {
                ...
                if ($is_formatting_function || $this->is_escaping_function($functionName) || $this->is_auto_escaped_function($functionName)) {
                    ...
                    if ('get_search_query' === strtolower($functionName)) {
                        ...
                        if (\false !== $escaped_param && 'true' !== $escaped_param['clean']) {
                            $this->phpcsFile->addError('Output from get_search_query() is unsafe due to $escaped parameter being set to "false".', $ptr, 'UnsafeSearchQuery');
                        }
                    }
                    continue;
                }
            ...
            $this->phpcsFile->addError("All output should be run through an escaping function (see the Security sections in the WordPress Developer Handbooks), found '%s'.", $ptr, $code, array($content));
        }
        return $end;
    }
    /**
    ...
    private function find_long_ternary($start, $end)
    {
        for ($i = $start; $i < $end; $i++) {
            ...
            if (isset($this->tokens[$i]['bracket_opener'], $this->tokens[$i]['bracket_closer']) && $i === $this->tokens[$i]['bracket_opener']) {
                $i = $this->tokens[$i]['bracket_closer'];
                continue;
            }
            // Skip past nested arrays, function calls and arbitrary groupings.
            if (\T_OPEN_PARENTHESIS === $this->tokens[$i]['code'] && isset($this->tokens[$i]['parenthesis_closer'])) {
                $i = $this->tokens[$i]['parenthesis_closer'];
                continue;
            }
            // Skip past closures, anonymous classes and anything else scope related.
            if (isset($this->tokens[$i]['scope_condition'], $this->tokens[$i]['scope_closer']) && $this->tokens[$i]['scope_condition'] === $i) {
             ...
             */
            if (Operators::isShortTernary($this->phpcsFile, $i) === \true) {
                return \false;
            }
            return $i;
        }
    ...
    private function walk_match_expression($stackPtr, $code)
    {
        ...
        do {
            ...
            for ($i = $item_start; $i <= $end; $i++) {
                ...
                if (\T_COMMA !== $this->tokens[$i]['code'] && $i !== $end) {
                    continue;
                }
                $item_end = $i;
                break;
