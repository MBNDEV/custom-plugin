...
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\SanitizationHelperTrait;
use _PhpScoperdd275a4cd119\WordPressCS\WordPress\Helpers\ValidationHelper;
...
class ValidatedSanitizedInputSniff extends Sniff
{
     ...
     */
    private $slashed_superglobals = array('$_COOKIE' => \true, '$_GET' => \true, '$_POST' => \true, '$_REQUEST' => \true, '$_SERVER' => \true);
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\T_DOUBLE_QUOTED_STRING === $this->tokens[$stackPtr]['code'] || \T_HEREDOC === $this->tokens[$stackPtr]['code']) {
            ...
            $interpolated_variables = array_map(static function ($embed) {
            ...
            $interpolated_superglobals = array_filter($interpolated_variables, static function ($var_name) {
        ...
        if ('$GLOBALS' === $this->tokens[$stackPtr]['content'] || Variables::isSuperglobalName($this->tokens[$stackPtr]['content']) === \false) {
            return;
        }
        ...
        if (\false === $validated) {
            $this->phpcsFile->addError('Detected usage of a possibly undefined superglobal array index: %s. Check that the array index exists before using it.', $stackPtr, 'InputNotValidated', $error_data);
        }
    ...
    public function add_unslash_error(File $phpcsFile, $stackPtr)
    {
        ...
        $phpcsFile->addError('%s not unslashed before sanitization. Use wp_unslash() or similar', $stackPtr, 'MissingUnslash', $error_data);
    }
}
