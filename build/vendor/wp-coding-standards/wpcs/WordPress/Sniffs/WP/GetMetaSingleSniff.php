...
final class GetMetaSingleSniff extends AbstractFunctionParameterSniff
{
     ...
     */
    private const GENERIC_META_FUNCTIONS_FORMAT = array('condition' => array('param_name' => 'meta_key', 'position' => 3), 'recommended' => array('param_name' => 'single', 'position' => 4));
    /**
     ...
     * @var array<string, array<string, int|string>>
     */
    private const SPECIFIC_META_FUNCTIONS_FORMAT = array('condition' => array('param_name' => 'key', 'position' => 2), 'recommended' => array('param_name' => 'single', 'position' => 3));
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        $message_data = array($condition['param_name'], $tokens[$stackPtr]['content'], $recommended['param_name']);
        $this->phpcsFile->addWarning('When passing the $%s parameter to %s(), it is recommended to also pass the $%s parameter to explicitly indicate whether a single value or multiple values are expected to be returned.', $stackPtr, 'Missing', $message_data);
    }
}
