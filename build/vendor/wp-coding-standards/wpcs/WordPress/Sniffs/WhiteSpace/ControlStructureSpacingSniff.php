 ...
 */
namespace _PhpScoperdd275a4cd119\WordPressCS\WordPress\Sniffs\WhiteSpace;
...
final class ControlStructureSpacingSniff extends Sniff
{
     ...
     * @var array
     */
    private $ignore_extra_space_after_open_paren = array(\T_DO => \true, \T_ELSE => \true, \T_TRY => \true, \T_FINALLY => \true);
    /**
    ...
    {
        return array(\T_IF, \T_WHILE, \T_FOREACH, \T_FOR, \T_SWITCH, \T_DO, \T_ELSE, \T_ELSEIF, \T_TRY, \T_CATCH, \T_FINALLY, \T_MATCH);
    }
    /**
    ...
    {
        if (isset($this->tokens[$stackPtr + 1]) && \T_WHITESPACE !== $this->tokens[$stackPtr + 1]['code'] && !(\T_ELSE === $this->tokens[$stackPtr]['code'] && \T_COLON === $this->tokens[$stackPtr + 1]['code'])) {
        ...
        if (\T_COLON !== $this->tokens[$parenthesisOpener]['code'] && $stackPtr + 1 === $parenthesisOpener) {
        ...
        if (isset($this->tokens[$parenthesisOpener]['parenthesis_closer'])) {
            ...
            if (\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
                ...
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser + 1]['code'] && (isset($scopeOpener) && \T_COLON !== $this->tokens[$scopeOpener]['code'])) {
            ...
            if (isset($this->tokens[$parenthesisOpener]['parenthesis_owner']) && (isset($scopeOpener) && $this->tokens[$parenthesisCloser]['line'] !== $this->tokens[$scopeOpener]['line'])) {
        ...
        if (\false !== $this->blank_line_check && isset($scopeOpener)) {
            ...
            $ignore = array(\T_DOC_COMMENT_OPEN_TAG => \true, \T_CLOSE_TAG => \true, \T_COMMENT => \true);
        ...
        if (\T_MATCH === $this->tokens[$stackPtr]['code']) {
            ...
            $next = $this->phpcsFile->findNext(Tokens::$emptyTokens, $scopeCloser + 1, null, \true);
            if (\false !== $next && (\T_SEMICOLON === $this->tokens[$next]['code'] || \T_COMMA === $this->tokens[$next]['code'])) {
                $scopeCloser = $next;
            }
        }
        ...
        if (isset($this->tokens[$trailingContent]['scope_condition']) && \T_CLOSE_CURLY_BRACKET === $this->tokens[$trailingContent]['code']) {
            ...
            if (isset(Collections::closedScopes()[$this->tokens[$owner]['code']]) === \true) {
